...
function getTime(date) {
  ...
  return `${String(newDate.getHours()).padStart(2, '0')}:${String(newDate.getMinutes()).padStart(2, '0')}:${String(newDate.getSeconds()).padStart(2, '0')}`;
}
...
function getNextFriday(date) {
  ...
  const daysToAdd = addDays();
  curDate.setUTCDate(curDate.getUTCDate() + daysToAdd);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const curDateStart = new Date(dateStart);
  const curDateEnd = new Date(dateEnd);
  const ms = curDateStart.getTime() - curDateEnd.getTime();
...
function formatDate(date) {
  ...
  let hh = curDate.getUTCHours();
  const mm = curDate.getUTCMinutes().toString().padStart(2, '0');
  const ss = curDate.getUTCSeconds().toString().padStart(2, '0');
...
function getWeekNumberByDate(date) {
  ...
  const diffMs = curDate - firstDayOfYear;

  const weekNumber = Math.ceil((diffMs / msInDay + dayOfWeek) / 7);
  ...
  while (curDate <= endDate) {
    if (isWorkDay) {
      for (let i = 0; i < countWorkDays && curDate <= endDate; i += 1) {
        schedule.push(ownFormatDate(curDate));
